...
```{r}
NOMBRE_COMPLETO = "Daniel Arturo Alpizar Ortega"
DIGITOS_MATRICULA = 1379692
```
...
- Densidad de la matriz (**NOTE-10**): 0.57
...
- support (mínimo) (**NOTE-12**): 0.01
- confidence (mínimo) (**NOTE-13**): 0.01
- items (máximo) (**NOTE-14**): 6
- tiempo de verificación de subsets (máximo) (**NOTE-15**): 0 s
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): Que no hay un producto con anterioridad a diapers para que estos sean comprados.
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 1/46
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 0 
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque no tiene como tal una regla asociada y su confianza es de 0. No hay relación
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Es el id que identifica a la sesión que se analiza y que acumula a todas las sesiones que se hicieron.
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Es para saber que canción es la que se escuchó 
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, ya que con las otras columnas te puede decir que tanta relación existe entre si. Por ejemplo, si la canción gustó y no se paro antes de reproducirse.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce con 914 
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 1575
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): que se tiene 14.62 sesiones en promedio
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Que debe de tener RHS y LHS, ya que mediante eso se conforman las reglas de asociación.
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Si se tiene un número bajo en cualquiera de las dos, ya sea support o confidence, esto quiere decir que la regla que se quiere analizar no tiene valor o no genera confianza. 
...
- Provee el enlace al video aquí: **NOTE-30** https://youtu.be/Ix5K2Wa8Fy4
